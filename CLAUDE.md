...
**Enemy shields + damage types**: `EncounterData.enemy_max_shield` / `shield_regen` / `shield_regen_delay` give each enemy a deflector profile; 0 capacity means an unshielded fight. The shield is a **separate health pool, not an absorber** -- damage that breaks it never spills onto the hull in the same hit, which is what makes card order a decision. `CardData.damage_type` (`DamageType.KINETIC` / `ION` / `PIERCING`) picks the multipliers in `card_battle.SHIELD_DAMAGE_MULT` / `HULL_DAMAGE_MULT`: kinetic is half-effective vs shields, ion is double vs shields but half vs hull, piercing skips the shield entirely. `CardKeyword.BOUNCES` (Torpedo) deals nothing at all while a shield stands; `SpecialEffect.PIERCE_NEXT` (Overload Coil) makes the next attack ignore it, and is spent in `_apply_attack_card()` so SHIELD_ECHO defense cards do not eat the charge. `SpecialAbility.SHIELD_BOOST` is an emergency recharge to full every `SHIELD_BOOST_INTERVAL` turns, layered on top of the normal regen -- it no longer adds flat shield of its own. Boarding is blocked while a shield is up. Adding shields to an enemy means lowering its `enemy_health`, or the fight just gets longer.
...
**Ship Upgrades** (`shipyard_panel.gd`): Compact panel with live 3D `ShipDisplay` and upgrade buttons. Hull repair (`GameManager.REPAIR_COST_PER_HP`, currently 30cr/HP), +5 max hull (200cr), +3 max shield (250cr), +2 cargo (300cr), plus card-adding upgrades, energy/hand size bonuses. Emits `shipyard_action` signal.
...
**Planet Background Parallax** (`depth_parallax.gd` + `shaders/depth_parallax.gdshader`): the hub background is displaced by a per-planet depth map (`assets/sprites/scenes/bg_<planet>_depth.png`, white = near) for a 2.5D look. Drivers: slow idle drift plus mouse position (desktop) or device tilt via `Input.get_gravity()` (touch). Hotspot layers are moved with the pixels under them via `track()`. Tapping a building flies the camera in (`zoom_to()`) before the overlay opens; `HubOverlayStack.emptied` triggers `zoom_out()`. A planet without a depth map keeps the static image. **Ambient lights** (`hub_ambience.gd`) sit right above the background: beacons, neon flicker, screen scan lines, traffic on paths and shuttle launches, defined per background in `hub_ambience_presets.gd` (texture-UV coordinates) and positioned through `DepthParallax.texture_to_screen()` so they sway and zoom with the image. Currently only Starport Alpha has a preset. Regenerate maps after changing a background: `tools/generate_depth_maps.py` (Depth Anything V2, needs a venv with torch/transformers).
